...
interface MultipartFileValidator {
...
@Service
class MultipartFileValidatorImpl : MultipartFileValidator {

    private val supportedExtensions = setOf("pdf")

    @Throws(IllegalArgumentException::class)
    override fun validate(files: List<MultipartFile>, fileNamesWithExtensions: List<String>) {
        validateNumberOfFiles(files)
        files.mapIndexed { index, file -> validate(file, fileNamesWithExtensions[index]) }
    }

    @Throws(IllegalArgumentException::class)
    override fun validate(file: MultipartFile, fileNameWithExtension: String) {
